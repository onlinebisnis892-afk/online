...
export async function executeFeature(feature: string, input: FeatureInput = {}): Promise<FeatureResult> {
  ...
  switch (feature) {
    ...
    case 'Rekonsiliasi Kas': {
      const branchId=asString(input.branchId); const from=new Date(asString(input.from)||new Date(Date.now()-86400000).toISOString()); const to=new Date(asString(input.to)||new Date().toISOString()); const rows=await db.cashEntry.findMany({where:{branchId,createdAt:{gte:from,lte:to}}}); const balance=rows.reduce((s: number,r: { type: string; amount: unknown })=>s+(r.type==='IN'?Number(r.amount):-Number(r.amount)),0); return {ok:true,feature,module:meta.module,message:'Rekonsiliasi selesai.',data:{balance,entries:rows}};
    }
    case 'Komisi': {
